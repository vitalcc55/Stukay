...
- local network flow, permission rationale и non-blocking connection UX вокруг него;
...
- `Settings` предоставляет рабочий pairing/connect/reconnect/disconnect flow для pairing payload input и local-network rationale без crash-path в `NotPaired`;
...
- app can request/reflect Android 16 local-network manual companion permission path without treating it as unconditional blocker;
...
- `Diagnostics` shows host bridge summary and dedicated recent host/connection events tail;
...
- `Settings` поддерживает pairing payload save/connect/reconnect/disconnect flow, не падает в `NotPaired`, и честно объясняет Android 16 / 17 local-network difference.
...
- проверить, что new logs и diagnostics не выводят pairing secret/token и не тащат backup-secret surface в auto-backup;
...
- done: собран active implementation plan и research подложка, добавлены typed `Host Bridge` models, введены runtime adapters и runtime graph для app state, добавлен pairing payload flow с guard-ами, manifest/network permission surface, connection-aware `Settings`, host summary в `Projects`, dedicated host diagnostics tail в `Diagnostics`, JVM tests для parser/repository transitions
